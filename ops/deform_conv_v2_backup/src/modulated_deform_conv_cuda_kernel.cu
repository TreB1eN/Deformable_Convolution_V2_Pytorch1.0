...
int ModulatedDeformConvForwardLaucher(
  ...
  const auto channel_per_deformable_group = num_channels; 
...
int ModulatedDeformConvBackwardLaucher(
  ...
  at::Tensor grad_mask,
  at::Tensor grad_im) {
  const auto offset_channels = 2 * kernel_h * kernel_w;
  const auto num_channels = data_im.size(1);
  const auto height = data_im.size(2);
  const auto width = data_im.size(3);
  const auto batch_size = data_im.size(0);
  ...
  const auto col2im_channel_per_deformable_group = num_channels;

  const auto col2im_coord_channel_per_deformable_group = data_col.size(0);

  const int col2im_num_kernels = batch_size * num_channels * kernel_h * kernel_w * width_col * height_col;

  const int col2im_coord_num_kernels = batch_size * height_col *  width_col * 2 * kernel_h * kernel_w;
