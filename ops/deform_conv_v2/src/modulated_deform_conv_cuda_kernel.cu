...
int ModulatedDeformConvForwardLaucher(
  ...
  // data_col : [channels * kernel_h * kernel_w, batch_size, height_col, width_col]
  // kernel : [out_channels, channels * kernel_h * kernel_w]
  // data_out : [out_channels, batch_size, height, width]
  at::Tensor data_im, 
  ...
  const auto num_channels = data_im.size(1);
  const auto height = data_im.size(2);
  const auto width = data_im.size(3);
  const auto batch_size = data_im.size(0);
  const auto channel_per_deformable_group = num_channels / deformable_group; 
  const auto height_col = data_col.size(2);
  const auto width_col = data_col.size(3);

  const int num_kernels = batch_size * num_channels * width_col * height_col;

  modulated_deformable_im2col_gpu_kernel<<<GET_BLOCKS(num_kernels), CUDA_NUM_THREADS>>>(
    num_kernels,
    data_im.data<float>(),
    data_offset.data<float>(),
    data_mask.data<float>(),
    height, width,
    kernel_h, kernel_w,
    pad_h, pad_w,
    stride_h, stride_w,
    ...
    batch_size, num_channels, deformable_group,
...
int ModulatedDeformConvBackwardLaucher(
  ...
  at::Tensor grad_mask,
  at::Tensor grad_im) {
  const auto offset_channels = 2 * kernel_h * kernel_w * deformable_group;
  const auto num_channels = data_im.size(1);
  const auto height = data_im.size(2);
  const auto width = data_im.size(3);
  const auto batch_size = data_im.size(0);
  ...
  const auto col2im_channel_per_deformable_group = num_channels / deformable_group;

  const auto col2im_coord_channel_per_deformable_group = data_col.size(0) / deformable_group;

  const int col2im_num_kernels = batch_size * num_channels * kernel_h * kernel_w * width_col * height_col;

  const int col2im_coord_num_kernels = batch_size * height_col *  width_col * 2 * kernel_h * kernel_w * deformable_group;
  

  modulated_deformable_col2im_gpu_kernel<<<GET_BLOCKS(col2im_num_kernels), CUDA_NUM_THREADS>>>(
    col2im_num_kernels,
    data_col.data<float>(),
    data_offset.data<float>(),
    data_mask.data<float>(),
    ...
    stride_h, stride_w,
    dilation_h, dilation_w,
    col2im_channel_per_deformable_group,
    batch_size, deformable_group,
    height_col, width_col,
    grad_im.data<float>());

  modulated_deformable_col2im_coord_gpu_kernel<<<GET_BLOCKS(col2im_coord_num_kernels), CUDA_NUM_THREADS>>>(
    col2im_coord_num_kernels,
    data_col.data<float>(),
    data_im.data<float>(),
    data_offset.data<float>(),
    data_mask.data<float>(),
    ...
    stride_h, stride_w,
    dilation_h, dilation_w,
    col2im_coord_channel_per_deformable_group,
    batch_size, offset_channels, deformable_group,
    height_col, width_col,
    grad_offset.data<float>(),
    grad_mask.data<float>());
